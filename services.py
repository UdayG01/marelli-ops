...
class FlexibleNutDetectionService:
    ...
    def __init__(self):
        ...
        # Load model FIRST
        self._load_model()
        logger.info("RenataAI model loaded: {}".format(self.model_path))
    ...
    def set_roi_points(self, points):
        """
        Set ROI points that define a polygon
    ...
    def detection_in_roi(self, detection, roi_polygon):
        """
        Check if a detection's center is within the ROI polygon
        
        Args:
            detection: Detection dict with 'bbox' key
            roi_polygon: numpy array of polygon vertices
        
        Returns:
            Boolean indicating if detection center is in ROI
        """
        bbox = detection['bbox']
        ...
        return self.point_in_polygon((center_x, center_y), roi_polygon)
    ...
    def filter_detections_by_roi(self, detections):
        ...
        """
        if not self.roi_enabled or self.roi_points is None:
            return detections
        
        filtered = []
        for detection in detections:
            if self.detection_in_roi(detection, self.roi_points):
                filtered.append(detection)
                logger.debug(f"Detection at bbox {detection['bbox']} is INSIDE ROI")
            else:
                logger.debug(f"Detection at bbox {detection['bbox']} is OUTSIDE ROI - filtered out")
        
        logger.info(f"ROI filtering: {len(detections)} -> {len(filtered)} detections")
        return filtered

    def draw_roi_on_image(self, image, roi_points=None, color=(255, 0, 255), thickness=3):
        """
        Draw ROI polygon on image
        
        Args:
            image: OpenCV image
            roi_points: Polygon points (uses self.roi_points if None)
            color: BGR color tuple for ROI outline
        ...
        # Draw polygon
        cv2.polylines(image, [roi_points], isClosed=True, color=color, thickness=thickness)
        
        # Draw vertices as circles for visibility
        for point in roi_points:
            cv2.circle(image, tuple(point), 5, color, -1)
            cv2.circle(image, tuple(point), 7, (255, 255, 255), 2)  # White border
        
        # Add semi-transparent overlay outside ROI
        overlay = image.copy()
        mask = np.zeros(image.shape[:2], dtype=np.uint8)
        cv2.fillPoly(mask, [roi_points], 255)
        
        # Darken area outside ROI
        outside_roi = cv2.bitwise_not(mask)
        overlay[outside_roi > 0] = (overlay[outside_roi > 0] * 0.3).astype(np.uint8)
        
        # Blend original and overlay
        result = cv2.addWeighted(image, 0.7, overlay, 0.3, 0)
        
        # Draw the polygon again on top for clarity
        cv2.polylines(result, [roi_points], isClosed=True, color=color, thickness=thickness)
        
        # Add ROI label
        cv2.putText(result, "ROI", (roi_points[0][0], roi_points[0][1] - 10),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
        
        return result
    
    def save_roi_config(self, filepath):
    ...
    def load_roi_config(self, filepath):
        ...
        try:
            ...
            if 'points' in roi_config:
                self.set_roi_points(roi_config['points'])
    ...
    def calculate_roi_area(self):
        """Calculate the area of the ROI polygon"""
        ...
        for i in range(n):
            j = (i + 1) % n
            area += self.roi_points[i][0] * self.roi_points[j][1]
            area -= self.roi_points[j][0] * self.roi_points[i][1]
    ...
    def get_roi_bounding_box(self):
        ...
        x_coords = self.roi_points[:, 0]
        y_coords = self.roi_points[:, 1]
    ...
    def _run_detection(self, image_path):
        ...
        try:
            ...
            if self.roi_enabled and self.roi_points is not None:
                logger.info(f"Applying ROI filter to {len(all_detections)} detections")
                all_detections = self.filter_detections_by_roi(all_detections)
                logger.info(f"After ROI filtering: {len(all_detections)} detections remain")    
    ...
    def _create_annotated_image_with_centers(self, image_path, detections, decision, image_id):
        ...
        # Draw ROI first (as background layer)
        if self.roi_enabled and self.roi_points is not None:
            image = self.draw_roi_on_image(image)
...
roi_set = enhanced_nut_detection_service.load_roi_config("roi_config.json")
if not roi_set:
    roi_points = [
        (800, 620),    # Top-left corner (padding above and left of top-left nut)
        (1470, 620),   # Top-right corner (padding above and right of top-right nut)
        (1470, 1690),  # Bottom-right corner (padding below and right of bottom nut)
        (800, 1690)    # Bottom-left corner (padding below and left of bottom nut)
    ]

    enhanced_nut_detection_service.set_roi_points(roi_points)
    # enhanced_nut_detection_service.save_roi_config("roi_config.json")