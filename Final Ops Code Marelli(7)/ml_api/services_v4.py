...
class FlexibleNutDetectionService:
    ...
    def __init__(self):
        ...
        # Create debug directory for region visualization
        self.debug_dir = os.path.join(settings.MEDIA_ROOT, 'inspections', 'debug')
    ...
    def visualize_region_selector(self, image_path: str) -> Dict[str, List[int]]:
        ...
        Displays image in same resolution as used by detection model.
        ...
        """
        points = []
        target_size = self.config['target_size']  # Get model's target size
        ...
        def click_event(event, x, y, flags, param):
            ...
            if event == cv2.EVENT_LBUTTONDOWN:
                ...
                # Start with a fresh copy of the resized image
                img_copy = img.copy()
                
                # Draw all points collected so far
                for i, pt in enumerate(points):
                    cv2.circle(img_copy, (pt[0], pt[1]), 5, (0, 255, 0), -1)
                    # Draw point number
        ...
        # Get model target size
        target_width, target_height = self.config['target_size']
            
        # Resize to exact model target size
        img = cv2.resize(img, (target_width, target_height), interpolation=cv2.INTER_AREA)
        img_copy = img.copy()  # Work with the resized image directly
        
        # Store display dimensions
        display_width, display_height = target_width, target_height
        ...
        while True:
            ...
            if key == 27:
                if len(points) == 4:
                    # Draw final preview with processed region
                    preview = img.copy()
                    # Convert points to original scale for preview
                    np_points = np.array([[int(x), int(y)] for x, y in points])
                    # Draw the polygon outline
                    cv2.polylines(preview, [np_points], True, (0, 255, 0), 2)
                    # Fill with semi-transparent overlay
                    overlay = preview.copy()
                    cv2.fillPoly(overlay, [np_points], (0, 255, 0, 128))
                    cv2.addWeighted(overlay, 0.3, preview, 0.7, 0, preview)
                    cv2.imshow('Preview of Processing Region', preview)
                    cv2.waitKey(2000)  # Show preview for 2 seconds
                    break
                else:
        ...
        if len(points) == 4:
            # Keep coordinates in display space - they're already at the right scale
            # because we resized the image to match the model's target size
            ...
            print("\nSelected Region Points (Processing Coordinates):")
            print(f"region = {{")
            print(f"    'points': [")
            ...
            print(f"Processing dimensions: {target_width}x{target_height}")
            print("Note: Points are in processing coordinate space")
        ...
        try:
            ...
            if region:
                try:
                    ...
                    # Save debug visualization of region
                    debug_image = image.copy()
                    cv2.rectangle(debug_image, (x1, y1), (x2, y2), (0, 255, 0), 2)
                    debug_path = os.path.join(self.debug_dir, f'region_debug_{image_id}.jpg')
                    cv2.imwrite(debug_path, debug_image)
                    
                    # Save cropped region for debug
                    region_path = os.path.join(self.debug_dir, f'region_crop_{image_id}.jpg')
                    cv2.imwrite(region_path, image_region)
