...
class HikrobotCameraManager:
    ...
    def connect(self):
        """Connect to the first available Hikrobot camera with enhanced diagnostics"""
        try:
            logger.info("[CAMERA] Starting camera connection process...")
            ...
            # Initialize SDK
            logger.info("[CAMERA] Initializing SDK...")
            ret = MvCamera.MV_CC_Initialize()
            if ret != 0:  # Assuming 0 is MV_OK equivalent
                logger.error(f"[CAMERA] SDK initialization failed: {ret}")
                return False, f"SDK initialization failed: {ret}"
            logger.info("[CAMERA] ✓ SDK initialized successfully")
            
            # Create camera object
            logger.info("[CAMERA] Creating camera object...")
            self.camera = MvCamera()
            logger.info("[CAMERA] ✓ Camera object created")
            
            # Enumerate devices with detailed logging
            logger.info("[CAMERA] Enumerating GigE devices...")
            self.device_list = MV_CC_DEVICE_INFO_LIST()
            
            # Focus on GigE devices first (since your target is 192.168.1.10)
            ret = self.camera.MV_CC_EnumDevices(MV_GIGE_DEVICE, self.device_list)
            if ret != 0:
                logger.error(f"[CAMERA] Failed to enumerate GigE devices. Error: {ret}")
                # Try USB devices as fallback
                logger.info("[CAMERA] Trying USB devices as fallback...")
                ret = self.camera.MV_CC_EnumDevices(MV_USB_DEVICE, self.device_list)
                if ret != 0:
                    logger.error(f"[CAMERA] Failed to enumerate USB devices. Error: {ret}")
            ...
            if self.device_list.nDeviceNum == 0:
                ...
                logger.info("[CAMERA] Troubleshooting checklist:")
                logger.info("[CAMERA]   - Camera power is ON")
                logger.info("[CAMERA]   - Network cable connected")
                logger.info("[CAMERA]   - Camera IP: 192.168.1.10")
                logger.info("[CAMERA]   - PC IP: 192.168.1.20")
                logger.info("[CAMERA]   - MVS software is CLOSED")
                logger.info("[CAMERA]   - Try: ping 192.168.1.10")
                MvCamera.MV_CC_Finalize()
                return False, "No Hikrobot cameras found"
            
            # Enhanced device selection logic - look for specific IP or use first device
            target_device_index = 0
            target_found = False
            
            # Log all found devices and look for target IP
            for i in range(self.device_list.nDeviceNum):
                device_info = self.device_list.pDeviceInfo[i].contents
                ...
                if device_info.nTLayerType == MV_GIGE_DEVICE:
                    ...
                    # Get device details
                    try:
                        model = gige_info.chModelName.decode('ascii').rstrip('\x00')
                        serial = gige_info.chSerialNumber.decode('ascii').rstrip('\x00')
                    except:
                        model = "Unknown"
                        serial = "Unknown"
                    
                    logger.info(f"[CAMERA]   Device {i+1}:")
                    logger.info(f"[CAMERA]     Model: {model}")
                    logger.info(f"[CAMERA]     Serial: {serial}")
                    logger.info(f"[CAMERA]     IP: {ip_str}")
                    
                    # Prefer camera at 192.168.1.10 if found
                    if ip_str == "192.168.1.10":
                        target_device_index = i
                        target_found = True
                        logger.info("[CAMERA]     ⭐ This is your target camera!")
                else:
                    # USB device
                    logger.info(f"[CAMERA]   Device {i+1}: USB Device")
            
            if target_found:
                logger.info(f"[CAMERA] Using target camera at 192.168.1.10 (Device {target_device_index + 1})")
            else:
                logger.info(f"[CAMERA] Target IP not found, using first available device (Device 1)")
            
            # Connect to selected camera
            stDeviceList = cast(self.device_list.pDeviceInfo[target_device_index], POINTER(MV_CC_DEVICE_INFO)).contents
            
            logger.info("[CAMERA] Creating device handle...")
            ret = self.camera.MV_CC_CreateHandle(stDeviceList)
            if ret != 0:
                logger.error(f"[CAMERA] Failed to create handle. Error: {ret}")
                MvCamera.MV_CC_Finalize()
                return False, f"Failed to create handle. Error: {ret}"
            logger.info("[CAMERA] ✓ Device handle created")
            
            logger.info("[CAMERA] Opening device...")
            ret = self.camera.MV_CC_OpenDevice(MV_ACCESS_Exclusive, 0)
            if ret != 0:
                logger.error(f"[CAMERA] Failed to open device. Error: {ret}")
                if ret == -2147483644:  # Common "device busy" error
                    logger.error("[CAMERA]     → Camera is probably being used by MVS software")
                    logger.error("[CAMERA]     → Close MVS completely and try again")
                self.camera.MV_CC_DestroyHandle()
                MvCamera.MV_CC_Finalize()
                return False, f"Failed to open device. Error: {ret}"
            logger.info("[CAMERA] ✓ Device opened successfully")
            
            # Test reading camera parameters to verify connection
            logger.info("[CAMERA] Testing camera parameter access...")
            try:
                width_info = MVCC_INTVALUE()
                height_info = MVCC_INTVALUE()
                
                ret1 = self.camera.MV_CC_GetIntValue("Width", width_info)
                ret2 = self.camera.MV_CC_GetIntValue("Height", height_info)
                
                if ret1 == 0 and ret2 == 0:
                    logger.info(f"[CAMERA] ✓ Camera resolution: {width_info.nCurValue} x {height_info.nCurValue}")
                else:
                    logger.info("[CAMERA] ✓ Basic connection confirmed (parameter read test skipped)")
            except Exception as e:
                logger.info("[CAMERA] ✓ Basic connection confirmed (parameter read test failed but connection OK)")
            
            # Configure camera for free-running mode
            logger.info("[CAMERA] Configuring camera for free-running mode...")
            ret = self.camera.MV_CC_SetEnumValue("TriggerMode", 0)
            if ret != 0:
                logger.warning(f"[CAMERA] Failed to disable trigger mode initially. Error: {ret}")
            else:
                logger.info("[CAMERA] ✓ Trigger mode disabled")
            
            # Start grabbing
            logger.info("[CAMERA] Starting image grabbing...")
            ret = self.camera.MV_CC_StartGrabbing()
            if ret != 0:
                logger.error(f"[CAMERA] Failed to start grabbing. Error: {ret}")
                ...
                MvCamera.MV_CC_Finalize()
                return False, f"Failed to start grabbing. Error: {ret}"
            logger.info("[CAMERA] ✓ Image grabbing started")
            ...
            logger.info("[CAMERA] 🎉 Camera connected successfully!")
            logger.info("[CAMERA] Camera is ready for image capture in free-running mode!")
            
            return True, "Camera connected successfully"
            
        except ImportError as e:
            logger.error(f"[CAMERA] Import error: {e}")
            logger.error("[CAMERA] Check:")
            logger.error("[CAMERA]   - MvImport folder exists")
            logger.error("[CAMERA]   - All .py files are in MvImport folder") 
            logger.error("[CAMERA]   - Run from the correct directory")
