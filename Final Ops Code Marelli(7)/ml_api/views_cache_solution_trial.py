...
class HikrobotCameraManager:
    ...
    def __init__(self):
        if not hasattr(self, 'initialized'):
            ...
            # Trigger mode functionality - FIXED
            ...
            # Trigger processing status tracking
            self.trigger_processing_status = {}
            self.processing_lock = threading.Lock()
            # NEW: Trigger frame buffer for immediate processing
            self.triggered_frames = []
            self.triggered_frames_lock = threading.Lock()
            # NEW: Final move queue for triggered inspections
    ...
    def _monitor_trigger_signal_fixed(self):
        """FIXED: Monitor trigger signal on Line 0 with proper frame handling"""
        logger.info("[TRIGGER] Started monitoring trigger signal on Line 0 (FIXED VERSION)")
        cleanup_counter = 0
        ...
        while not self.stop_monitoring.is_set() and self.is_monitoring_trigger:
            try:
                ...
                # Perform cleanup every 10 triggers
                if cleanup_counter >= 10:
                    self._cleanup_trigger_data()
                    cleanup_counter = 0
                if not self.is_connected or not self.is_trigger_mode:
                    break
                
                # FIXED: Capture triggered frame with proper timeout
                ...
                if frame is not None:
                    ...
                    # Store triggered frame for immediate processing
                    with self.triggered_frames_lock:
                    ...
                    # Process triggered image asynchronously
                    self._save_and_process_triggered_image_enhanced(frame, self.trigger_count, self.last_trigger_time)
                
                # FIXED: Smaller delay for more responsive triggering
                time.sleep(0.001)  # 1ms - much more responsive
    ...
    def _save_and_process_triggered_image_enhanced(self, frame, trigger_count, timestamp):
        ...
        try:
            ...
            if success:
                ...
                # Update status with file path and clear old data
                with self.processing_lock:
                    # Clean up old entries first
                    current_time = datetime.now()
                    old_triggers = [t for t, s in self.trigger_processing_status.items() 
                                  if s.get('complete') and 
                                  (current_time - datetime.fromisoformat(s.get('completed_at', current_time.isoformat()))).total_seconds() > 3600]
                    
                    for old_trigger in old_triggers:
                        del self.trigger_processing_status[old_trigger]
                    
                    # Update current trigger status
